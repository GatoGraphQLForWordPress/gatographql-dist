...
class GraphQLDataStructureFormatter extends MirrorQueryDataStructureFormatter
{
    ...
    public function getFormattedData(array $data) : array
    {
        ...
        if (isset($resultData[self::ADDITIONAL_FEEDBACK])) {
            foreach ($resultData[self::ADDITIONAL_FEEDBACK][Response::GENERAL_FEEDBACK] ?? [] as $category => $feedbackEntries) {
                $data[Response::GENERAL_FEEDBACK][$category] = \array_merge($data[Response::GENERAL_FEEDBACK][$category] ?? [], $feedbackEntries);
            }
            foreach ($resultData[self::ADDITIONAL_FEEDBACK][Response::DOCUMENT_FEEDBACK] ?? [] as $category => $feedbackEntries) {
                $data[Response::DOCUMENT_FEEDBACK][$category] = \array_merge($data[Response::DOCUMENT_FEEDBACK][$category] ?? [], $feedbackEntries);
            }
            foreach ($resultData[self::ADDITIONAL_FEEDBACK][Response::SCHEMA_FEEDBACK] ?? [] as $category => $feedbackEntries) {
                $data[Response::SCHEMA_FEEDBACK][$category] = \array_merge($data[Response::SCHEMA_FEEDBACK][$category] ?? [], $feedbackEntries);
            }
            foreach ($resultData[self::ADDITIONAL_FEEDBACK][Response::OBJECT_FEEDBACK] ?? [] as $category => $feedbackEntries) {
                $data[Response::OBJECT_FEEDBACK][$category] = \array_merge($data[Response::OBJECT_FEEDBACK][$category] ?? [], $feedbackEntries);
            }
         ...
         */
        $errors = \array_merge($this->reformatGeneralEntries($data[Response::GENERAL_FEEDBACK][FeedbackCategories::ERROR] ?? []), $this->reformatDocumentEntries($data[Response::DOCUMENT_FEEDBACK][FeedbackCategories::ERROR] ?? []), $this->reformatSchemaEntries($data[Response::SCHEMA_FEEDBACK][FeedbackCategories::ERROR] ?? []), $this->reformatObjectEntries($data[Response::OBJECT_FEEDBACK][FeedbackCategories::ERROR] ?? []));
        if ($errors !== []) {
            $ret['errors'] = $errors;
        }
    ...
    protected function maybeAddTopLevelExtensionsEntryToResponse(array &$ret, array $data) : void
    {
        ...
        // Deprecations
        $deprecations = \array_merge($this->reformatSchemaEntries($data[Response::SCHEMA_FEEDBACK][FeedbackCategories::DEPRECATION] ?? []), $this->reformatObjectEntries($data[Response::OBJECT_FEEDBACK][FeedbackCategories::DEPRECATION] ?? []));
        if ($deprecations !== []) {
            $ret['extensions']['deprecations'] = $deprecations;
        }
        // Warnings
        $warnings = \array_merge($this->reformatGeneralEntries($data[Response::GENERAL_FEEDBACK][FeedbackCategories::WARNING] ?? []), $this->reformatDocumentEntries($data[Response::DOCUMENT_FEEDBACK][FeedbackCategories::WARNING] ?? []), $this->reformatSchemaEntries($data[Response::SCHEMA_FEEDBACK][FeedbackCategories::WARNING] ?? []), $this->reformatObjectEntries($data[Response::OBJECT_FEEDBACK][FeedbackCategories::WARNING] ?? []));
        if ($warnings !== []) {
            $ret['extensions']['warnings'] = $warnings;
        }
        // Logs
        $logs = \array_merge($this->reformatGeneralEntries($data[Response::GENERAL_FEEDBACK][FeedbackCategories::LOG] ?? []), $this->reformatDocumentEntries($data[Response::DOCUMENT_FEEDBACK][FeedbackCategories::LOG] ?? []), $this->reformatSchemaEntries($data[Response::SCHEMA_FEEDBACK][FeedbackCategories::LOG] ?? []), $this->reformatObjectEntries($data[Response::OBJECT_FEEDBACK][FeedbackCategories::LOG] ?? []));
        if ($logs !== []) {
            $ret['extensions']['logs'] = $logs;
        }
        // Notices
        $notices = \array_merge($this->reformatGeneralEntries($data[Response::GENERAL_FEEDBACK][FeedbackCategories::NOTICE] ?? []), $this->reformatDocumentEntries($data[Response::DOCUMENT_FEEDBACK][FeedbackCategories::NOTICE] ?? []), $this->reformatSchemaEntries($data[Response::SCHEMA_FEEDBACK][FeedbackCategories::NOTICE] ?? []), $this->reformatObjectEntries($data[Response::OBJECT_FEEDBACK][FeedbackCategories::NOTICE] ?? []));
        if ($notices !== []) {
            $ret['extensions']['notices'] = $notices;
        }
        // Suggestions
        $suggestions = \array_merge($this->reformatGeneralEntries($data[Response::GENERAL_FEEDBACK][FeedbackCategories::SUGGESTION] ?? []), $this->reformatDocumentEntries($data[Response::DOCUMENT_FEEDBACK][FeedbackCategories::SUGGESTION] ?? []), $this->reformatSchemaEntries($data[Response::SCHEMA_FEEDBACK][FeedbackCategories::SUGGESTION] ?? []), $this->reformatObjectEntries($data[Response::OBJECT_FEEDBACK][FeedbackCategories::SUGGESTION] ?? []));
        if ($suggestions !== []) {
    ...
    {
        $entry = ['message' => $item[Tokens::MESSAGE]];
    ...
    protected function getDocumentEntry(array $item) : array
    {
        ...
        if ($extensions = \array_merge($this->getDocumentEntryExtensions($item), $item[Tokens::EXTENSIONS] ?? [])) {
            $entry['extensions'] = $extensions;
        }
        return $entry;
    }
    ...
    protected function getSchemaEntry(string $typeOutputKey, array $item) : array
    {
        ...
        if ($extensions = \array_merge($this->getSchemaEntryExtensions($typeOutputKey, $item), $item[Tokens::EXTENSIONS] ?? [])) {
            $entry['extensions'] = $extensions;
        }
        return $entry;
    }
    ...
    protected function getObjectEntry(string $typeOutputKey, array $item) : array
    {
        ...
        if ($extensions = \array_merge($this->getObjectEntryExtensions($typeOutputKey, $item), $item[Tokens::EXTENSIONS] ?? [])) {
            $entry['extensions'] = $extensions;
        }
        return $entry;
    }
    ...
    protected function validateObjectData(FieldInterface $field, string $typeOutputKey, array &$sourceRet, array &$resolvedObjectRet, SplObjectStorage $resolvedObject, $objectID) : bool
    {
        ...
        if (\array_key_exists($field->getOutputKey(), $resolvedObjectRet)) {
             ...
             */
            $differentFieldsWithSameOutputKeyForObject = \array_values(\array_filter($previouslyResolvedFieldsForObject, function (FieldInterface $previousField) use($field, $fragments) : bool {
        ...
        if ($sameOutputKeyField !== null) {
             ...
             */
            $item = $this->getFeedbackEntryManager()->formatObjectOrSchemaFeedbackCommonEntry($field, null, [], new FeedbackItemResolution(GraphQLSpecErrorFeedbackItemProvider::class, GraphQLSpecErrorFeedbackItemProvider::E_5_3_2, [$field->asFieldOutputQueryString(), $objectID, $sameOutputKeyField->asFieldOutputQueryString(), $field->getOutputKey()]), [$objectID]);
        ...
        return parent::validateObjectData($field, $typeOutputKey, $sourceRet, $resolvedObjectRet, $resolvedObject, $objectID);
    }
    /**
