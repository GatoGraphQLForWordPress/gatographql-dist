...
class MirrorQueryDataStructureFormatter extends AbstractJSONDataStructureFormatter
{
    ...
    protected function getFieldsFromExecutableDocument(ExecutableDocument $executableDocument) : array
    {
        ...
        return $this->getFieldsFromOperations($operations, $executableDocument->getDocument()->getFragments());
    }
    /**
     ...
     * @return FieldInterface[]
     */
    protected function getFieldsFromOperations(array $operations, array $fragments) : array
    {
        $fields = [];
        $astHelperService = $this->getASTHelperService();
        foreach ($operations as $operation) {
            $fields = \array_merge($fields, $astHelperService->getAllFieldsFromFieldsOrFragmentBonds($operation->getFieldsOrFragmentBonds(), $fragments));
        }
        return $fields;
    }
    /**
    ...
    private function addObjectData(array &$sourceRet, ?array &$resolvedObjectRet, array $fields, array &$databases, array &$unionTypeOutputKeyIDs, $objectID, string $objectKeyPath, array &$typeOutputKeyPaths, bool $concatenateField) : void
    {
        ...
        if ($concatenateField) {
            list($typeOutputKey, $objectID) = UnionTypeHelpers::extractObjectTypeAndID(
        ...
        foreach ($fields as $field) {
             ...
             */
            $validObjectData = $this->validateObjectData($field, $typeOutputKey, $sourceRet, $resolvedObjectRet, $resolvedObject, $objectID);
            if (!$validObjectData) {
                continue;
            }
             ...
             */
            if ($resolvedObject[$relationalField] === null && !$skipAddingDataForType) {
            ...
            $relationalNestedFields = $astHelperService->getAllFieldsFromFieldsOrFragmentBonds($relationalField->getFieldsOrFragmentBonds(), $fragments);
            ...
            if (!empty($resolvedObjectNestedPropertyRet)) {
                ...
                if (!\is_array($resolvedObjectNestedPropertyRet)) {
                    $resolvedObjectRet[$relationalFieldOutputKey] = [FieldOutputKeys::ID => $resolvedObjectRet[$relationalFieldOutputKey]];
                } else {
    ...
    {
        return \in_array($typeOutputKey, [$this->getSuperRootObjectTypeResolver()->getTypeOutputKey()]);
    }
    /**
