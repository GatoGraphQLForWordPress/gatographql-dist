...
class GraphQLSchemaHelpers
{
     ...
     */
    public static function encodeValueUsingGraphQLLanguage(mixed $value, bool $isEnumType) : string
    {
        if (\is_array($value)) {
            if (\array_values($value) === $value) {
                return \sprintf('[%s]', \implode(', ', \array_map(fn(mixed $listItemValue) => self::encodeValueUsingGraphQLLanguage($listItemValue, $isEnumType), $value)));
            }
            $inputObjectFieldEntries = [];
            foreach ($value as $inputFieldName => $inputFieldValue) {
                $inputObjectFieldEntries[] = \sprintf('%s: %s', $inputFieldName, self::encodeValueUsingGraphQLLanguage($inputFieldValue, \false));
            }
            return \sprintf('{%s}', \implode(', ', $inputObjectFieldEntries));
        }
