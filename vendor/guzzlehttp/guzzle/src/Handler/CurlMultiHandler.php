    ...
    private static function triggerConflictingCurlMultiOptionDeprecations(array $options) : void
    {
        ...
        foreach ($options as $option => $_) {
            if (\array_key_exists($option, $conflictingOptions)) {
                \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', $sinceOverrides[$option] ?? '7.14', \sprintf('Passing %s in the cURL multi handler "options" is deprecated; guzzlehttp/guzzle 8.0 will reject this option. Use %s instead.', self::formatCurlMultiOption($option), $conflictingOptions[$option]));
            }
        }
    }
    /**
