...
use GatoExternalPrefixByGatoGraphQL\GuzzleHttp\TransportSharing;
...
class CurlMultiHandler
{
    ...
    public function __construct(array $options = [])
    {
        foreach ($options as $name => $_) {
            if (!isset(self::KNOWN_CONSTRUCTOR_OPTIONS[$name])) {
                \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.14', \sprintf('The "%s" CurlMultiHandler constructor option is unknown; guzzlehttp/guzzle 8.0 will reject unknown constructor options.', (string) $name));
            }
        }
        ...
        if (isset($options['select_timeout'])) {
            ...
            if (!\is_int($selectTimeout) && !\is_float($selectTimeout) && (!\is_string($selectTimeout) || !\is_numeric($selectTimeout))) {
                \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.14', 'Passing a non-numeric "select_timeout" CurlMultiHandler option is deprecated; guzzlehttp/guzzle 8.0 will reject it.');
            } else {
                $seconds = (float) $selectTimeout;
                if (!\is_finite($seconds) || $seconds < 0 || $seconds > 0 && (int) ($seconds * 1000) === 0) {
                    \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.14', 'Passing a "select_timeout" CurlMultiHandler option that is not 0 or greater than or equal to 0.001 seconds is deprecated; guzzlehttp/guzzle 8.0 will reject it.');
                }
            }
            $this->selectTimeout = $selectTimeout;
        } elseif ($selectTimeout = Utils::getenv('GUZZLE_CURL_SELECT_TIMEOUT')) {
            \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.2', 'The GUZZLE_CURL_SELECT_TIMEOUT environment variable is deprecated; use the "select_timeout" option instead.');
    ...
    private static function triggerConflictingCurlMultiOptionDeprecations(array $options) : void
    {
        ...
        foreach ($options as $option => $_) {
            if (\array_key_exists($option, $conflictingOptions)) {
                \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.14', \sprintf('Passing %s in the cURL multi handler "options" is deprecated; guzzlehttp/guzzle 8.0 will reject this option. Use %s instead.', self::formatCurlMultiOption($option), $conflictingOptions[$option]));
            }
        }
    }
    /**
    ...
    private function applyProxyTunnelOwnership(EasyHandle $easy) : void
    {
        ...
        if ($this->handles === [] && !$this->executingMulti && 0 === $this->messageProcessingDepth && $this->deferredCancels === []) {
    ...
    {
        if (!\is_int($id)) {
            \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.4', 'Not passing an int to %s::%s() is deprecated and will cause an error in 8.0.', __CLASS__, __FUNCTION__);
        }
