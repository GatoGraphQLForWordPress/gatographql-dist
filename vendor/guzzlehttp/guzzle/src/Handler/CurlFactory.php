...
use GatoExternalPrefixByGatoGraphQL\GuzzleHttp\Psr7\Uri;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options) : void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function triggerUnsupportedCurlOptionDeprecations(array $options) : void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject raw cURL options outside the built-in cURL handlers\' allow-list.', self::formatCurlOption($option)));
        }
    }
    private static function triggerUnsupportedRequestOptionDeprecations(array $options) : void
    {
        if (\array_key_exists('stream_context', $options)) {
            \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "stream_context" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because cURL handlers ignore PHP stream context options.');
        }
    }
    /**
    ...
    private static function isProxyAuthorizationHeaderLine(string $header) : bool
    {
        ...
        return \strtr(\trim(\substr($header, 0, $length), " \n\r\t\x00\v"), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz') === 'proxy-authorization';
    }
    ...
    private function applyMethod(EasyHandle $easy, array &$conf) : void
    {
        if ($easy->request->getMethod() === 'HEAD') {
            ...
            if (\strtr(\trim($easy->request->getHeaderLine('Expect'), " \n\r\t\x00\v"), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz') === '100-continue') {
    ...
    private static function headersFromTrailerLines(array $lines) : array
    {
        ...
        foreach ($lines as $line) {
            ...
            $name = \strtr(\trim($name, " \n\r\t\x00\v"), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz');
