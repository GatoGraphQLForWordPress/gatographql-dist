...
use GatoExternalPrefixByGatoGraphQL\GuzzleHttp\Handler\CurlShareHandleState;
use GatoExternalPrefixByGatoGraphQL\GuzzleHttp\Handler\CurlVersion;
...
class Client implements ClientInterface, \GatoExternalPrefixByGatoGraphQL\Psr\Http\Client\ClientInterface
{
    ...
    private static function warnAboutRequestLevelHandler(array $options) : void
    {
        ...
        \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing the "handler" request option is deprecated; guzzlehttp/guzzle 8.0 will ignore request-level handlers. Configure the handler when creating the Client, or use a separate Client instance for requests that need a different handler.');
    }
    ...
    {
        \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', $since, 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
    ...
    private function transfer(RequestInterface $request, array $options) : PromiseInterface
    {
        ...
        if ('' === $protocolVersion) {
            ...
            \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with a malformed protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject malformed protocol versions.');
        }
        ...
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                ...
                case 'digest':
                    ...
                    break;
                case 'ntlm':
                    \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing "ntlm" as the built-in auth type is deprecated; guzzlehttp/guzzle 8.0 will no longer apply NTLM through the "auth" request option. NTLM is also deprecated by curl/libcurl and may be unavailable in current or future libcurl builds. Avoid NTLM; if you must use it temporarily, configure cURL HTTP authentication options directly with a libcurl build that still supports NTLM.');
    ...
    private static function createBodyStream($body) : StreamInterface
    {
        ...
        if (\is_scalar($body)) {
            \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-string scalar to the "body" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-string scalar bodies.');
    ...
    private static function normalizeNonFiniteFloats(array $values, string $option) : array
    {
        foreach ($values as $key => $value) {
            if (\is_array($value)) {
                ...
                \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-finite float in the "%s" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-finite floats.', $option);
    ...
    {
        if ('' === $version) {
            \GatoExternalPrefixByGatoGraphQL\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing an empty "version" request option is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            return '1.1';
        }
