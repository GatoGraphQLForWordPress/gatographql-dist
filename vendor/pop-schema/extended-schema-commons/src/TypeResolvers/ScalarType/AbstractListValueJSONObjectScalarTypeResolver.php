...
abstract class AbstractListValueJSONObjectScalarTypeResolver extends AbstractJSONObjectScalarTypeResolver
{
    ...
    {
        return \sprintf($this->__('Custom scalar representing a JSON Object where values are lists (of anything)%s', 'gatographql'), $this->canValueBeNullable() ? $this->__(' or null', 'gatographql') : '');
    }
    public function coerceValue(string|int|float|bool|stdClass $inputValue, AstInterface $astNode, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : string|int|float|bool|object|null
    {
        ...
        foreach ($inputValueArray as $key => $value) {
            ...
            if (!(\is_array($value) || $canValueBeNullable && $value === null)) {
                $objectTypeFieldResolutionFeedbackStore->addError(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(InputValueCoercionErrorFeedbackItemProvider::class, InputValueCoercionErrorFeedbackItemProvider::E3, [$this->getMaybeNamespacedTypeName(), $this->getOutputService()->jsonEncodeArrayOrStdClassValue($inputValue)]), $astNode));
                return null;
            }
    ...
    protected abstract function canValueBeNullable() : bool;
}
