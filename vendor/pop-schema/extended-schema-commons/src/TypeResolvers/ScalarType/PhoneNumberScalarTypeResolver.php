...
class PhoneNumberScalarTypeResolver extends AbstractScalarTypeResolver
{
    ...
    public function coerceValue($inputValue, AstInterface $astNode, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore)
    {
        ...
        if (\preg_match('/(\\+{1}[0-9]{1,3}[0-9]{8,9})/', $inputValue) !== 1) {
            $objectTypeFieldResolutionFeedbackStore->addError(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(InputValueCoercionErrorFeedbackItemProvider::class, InputValueCoercionErrorFeedbackItemProvider::E1, [$this->getMaybeNamespacedTypeName()]), $astNode));
            return null;
        }
        return $inputValue;
    }
