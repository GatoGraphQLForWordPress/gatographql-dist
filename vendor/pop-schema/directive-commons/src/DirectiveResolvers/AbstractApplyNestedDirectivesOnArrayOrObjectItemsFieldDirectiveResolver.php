...
abstract class AbstractApplyNestedDirectivesOnArrayOrObjectItemsFieldDirectiveResolver extends AbstractGlobalMetaFieldDirectiveResolver implements DynamicVariableDefinerFieldDirectiveResolverInterface
{
    ...
    {
        return \array_merge(parent::getDirectiveArgNameTypeResolvers($relationalTypeResolver), [$this->getPassValueOnwardsAsVariableArgumentName() => $this->getStringScalarTypeResolver()], $this->passKeyOnwardsAsVariable() ? [$this->getPassKeyOnwardsAsVariableArgumentName() => $this->getStringScalarTypeResolver()] : []);
    }
    ...
    public function resolveDirective(RelationalTypeResolverInterface $relationalTypeResolver, array $idFieldSet, FieldDataAccessProviderInterface $fieldDataAccessProvider, array $succeedingPipelineFieldDirectiveResolvers, array $idObjects, array $unionTypeOutputKeyIDs, array $previouslyResolvedIDFieldValues, array &$succeedingPipelineIDFieldSet, array &$succeedingPipelineFieldDataAccessProviders, array &$resolvedIDFieldValues, array &$messages, EngineIterationFeedbackStore $engineIterationFeedbackStore) : void
    {
         ...
         */
        $originalFieldTypeModifiersByField = $decreaseFieldTypeModifiersCardinalityForSerialization ? clone $fieldTypeModifiersByField : null;
        /** @var SplObjectStorage<FieldInterface,int|null> */
        ...
        foreach ($idFieldSet as $id => $fieldSet) {
            ...
            foreach ($fieldSet->fields as $field) {
                ...
                if (!$hasIDFieldBeenResolved && !(isset($previouslyResolvedIDFieldValues[$typeOutputKey][$id]) && $previouslyResolvedIDFieldValues[$typeOutputKey][$id]->offsetExists($field))) {
                    $this->processObjectFailure($relationalTypeResolver, new FeedbackItemResolution(EngineErrorFeedbackItemProvider::class, EngineErrorFeedbackItemProvider::E2, [$field->getOutputKey(), $id]), [$id => new EngineIterationFieldSet([$field])], $succeedingPipelineIDFieldSet, $this->directive, $resolvedIDFieldValues, $engineIterationFeedbackStore);
                    continue;
                }
                $value = $hasIDFieldBeenResolved ? $resolvedIDFieldValues[$id][$field] : $previouslyResolvedIDFieldValues[$typeOutputKey][$id][$field];
                ...
                if (!$this->validateInputValueType($value)) {
                    $this->processObjectFailure($relationalTypeResolver, $this->getItemValueValidationFailedFeedbackItemResolution($field, $id), [$id => new EngineIterationFieldSet([$field])], $succeedingPipelineIDFieldSet, $this->directive, $resolvedIDFieldValues, $engineIterationFeedbackStore);
                    continue;
                }
                if ($resolveDirectiveArgsOnObject) {
                    ...
                    if ($directiveArgs === null) {
                        if ($setFieldAsNullIfDirectiveFailed) {
                            $this->removeIDFieldSet($succeedingPipelineIDFieldSet, [$id => new EngineIterationFieldSet([$field])]);
                            $this->setFieldResponseValueAsNull($resolvedIDFieldValues, [$id => new EngineIterationFieldSet([$field])]);
                        }
                        continue;
                    }
                    $passValueOnwardsAs = $directiveArgs[$passValueOnwardsAsVariableArgumentName] ?? null;
                ...
                $arrayOrObject = $value;
                $arrayItems = $this->getArrayItems($arrayOrObject, $object, $id, $field, $relationalTypeResolver, $idObjects, $previouslyResolvedIDFieldValues, $succeedingPipelineIDFieldSet, $resolvedIDFieldValues, $messages, $engineIterationFeedbackStore);
                if ($arrayItems !== []) {
                    $execute = \true;
                    $nestedFieldDataAccessProvider ??= clone $fieldDataAccessProvider;
                    if ($decreaseFieldTypeModifiersCardinalityForSerialization && !$currentFieldTypeModifiersByField->offsetExists($field)) {
                    ...
                    $parentFieldTypeModifiers = $hasParentFieldTypeModifiers ? $fieldTypeModifiersByField[$field] : null;
                    ...
                    foreach ($arrayItems as $key => &$value) {
                        ...
                        if (!empty($passValueOnwardsAs)) {
                            ...
                            $objectResolvedDynamicVariablesService->setObjectResolvedDynamicVariableInAppState($relationalTypeResolver, $arrayItemField, $object, $id, $value, \true, $passValueOnwardsAs, [$arrayItemField], $valueArgument, $this->directive, $engineIterationFeedbackStore);
                        }
                     ...
                     */
                    $objectResolvedDynamicVariablesService->copyObjectResolvedDynamicVariablesFromFieldToFieldsInAppState($field, \array_column($arrayItemEntriesForField, 0));
        ...
        if ($execute) {
            ...
            if ($nestErrorsInMetaDirectives && ($objectResolutionFeedbackStoreErrors !== [] || $schemaFeedbackStoreErrors !== [])) {
                ...
                if ($schemaFeedbackStoreErrors !== []) {
                    ...
                    $engineIterationFeedbackStore->schemaFeedbackStore->addError(new SchemaFeedback(new FeedbackItemResolution(ErrorFeedbackItemProvider::class, ErrorFeedbackItemProvider::E18, [$this->directive->asQueryString()], $schemaFeedbackStoreErrors), $this->directive, $relationalTypeResolver, $fields));
                }
                if ($objectResolutionFeedbackStoreErrors !== []) {
                    $engineIterationFeedbackStore->objectResolutionFeedbackStore->addError(new ObjectResolutionFeedback(new FeedbackItemResolution(ErrorFeedbackItemProvider::class, ErrorFeedbackItemProvider::E18, [$this->directive->asQueryString()], $objectResolutionFeedbackStoreErrors), $this->directive, $relationalTypeResolver, $this->directive, $idFieldSet));
                }
                if ($setFieldAsNullIfDirectiveFailed) {
                    $this->removeIDFieldSet($succeedingPipelineIDFieldSet, $idFieldSet);
                    $this->setFieldResponseValueAsNull($resolvedIDFieldValues, $idFieldSet);
            ...
            foreach ($arrayItemEntriesByIDField as $id => $fieldEntries) {
                ...
                foreach ($fieldEntries as $field) {
                    ...
                    $this->addProcessedItemsBackToResolvedIDFieldValues($relationalTypeResolver, $resolvedIDFieldValues, $engineIterationFeedbackStore, $id, $field, $arrayItemKeyValues);
                }
            }
        }
        /**
     ...
     */
    protected function decreaseFieldTypeModifiersForSerializationInAppState(SplObjectStorage $fieldTypeModifiersByField, ObjectTypeResolverInterface $targetObjectTypeResolver, FieldInterface $field) : ?int
    {
        /** @var int|null */
    ...
    protected function getArrayItemField(FieldInterface $field, string $arrayItemAlias) : FieldInterface
    {
        if (!$this->arrayItemFieldInstanceContainer->offsetExists($field) || !isset($this->arrayItemFieldInstanceContainer[$field][$arrayItemAlias])) {
            ...
            $arrayItemFieldInstanceContainerSplObjectStorage[$arrayItemAlias] = $field instanceof RelationalField ? new RelationalField($field->getName(), $arrayItemAlias, $field->getArguments(), $field->getFieldsOrFragmentBonds(), $field->getDirectives(), new RuntimeLocation($field)) : new LeafField($field->getName(), $arrayItemAlias, $field->getArguments(), $field->getDirectives(), new RuntimeLocation($field));
    ...
    {
        $this->addProcessedItemsBackToResolvedIDFieldValues($relationalTypeResolver, $resolvedIDFieldValues, $engineIterationFeedbackStore, $id, $field, [[$arrayItemKey, $arrayItemValue]]);
    }
    /**
     ...
     */
    protected function addProcessedItemsBackToResolvedIDFieldValues(RelationalTypeResolverInterface $relationalTypeResolver, array &$resolvedIDFieldValues, EngineIterationFeedbackStore $engineIterationFeedbackStore, string|int $id, FieldInterface $field, array $arrayItemKeyValues) : void
    {
        ...
        foreach ($arrayItemKeyValues as [$arrayItemKey, $arrayItemValue]) {
            $this->setProcessedArrayItemValue($relationalTypeResolver, $engineIterationFeedbackStore, $id, $field, $arrayItemKey, $arrayItemValue, $value);
        }
     ...
     */
    protected function setProcessedArrayItemValue(RelationalTypeResolverInterface $relationalTypeResolver, EngineIterationFeedbackStore $engineIterationFeedbackStore, string|int $id, FieldInterface $field, int|string $arrayItemKey, mixed $arrayItemValue, array|stdClass &$value) : void
    {
        if (\is_array($value)) {
    ...
    {
        return \array_merge([$this->getPassValueOnwardsAsVariableArgumentName()], $this->passKeyOnwardsAsVariable() ? [$this->getPassKeyOnwardsAsVariableArgumentName()] : []);
    }
    ...
    public final function mustResolveDynamicVariableOnObject() : bool
    {
        return \true;
    }
}
