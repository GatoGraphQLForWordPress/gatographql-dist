...
class Logger extends AbstractBasicService implements \PoPSchema\Logger\Log\LoggerInterface
{
    ...
    protected final function getSystemLogger() : \PoPSchema\Logger\Log\SystemLoggerInterface
    {
        if ($this->systemLogger === null) {
            ...
            $systemLogger = $this->instanceManager->getInstance(\PoPSchema\Logger\Log\SystemLoggerInterface::class);
    ...
    protected function getMessageWithLogSeverity(string $severity, string $message) : string
    {
        ...
        $message = \sprintf($this->__('%s %s', 'gatographql'), $messageSeverity, $message);
        if ($this->addLoggerSignToMessage()) {
            $message = \sprintf($this->__('%s %s', 'gatographql'), $this->getLoggerSeveritySign($severity), $message);
        }
        return $message;
    }
    ...
    protected function maybeCreateLogFile(string $filename) : bool
    {
        ...
        if (!\is_dir($dir) && @\mkdir($dir, 0777, \true) === \false) {
