...
{
    use \PoPCMSSchema\MenuMutations\MutationResolvers\DeleteMenuMutationResolverTrait;
    ...
    protected final function getMenuTypeMutationAPI() : MenuTypeMutationAPIInterface
    {
     ...
     */
    protected function validateMenuExists(string|int|null $menuID, FieldDataAccessorInterface $fieldDataAccessor, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : void
    {
        if (!$menuID || $this->getMenuTypeAPI()->getMenu($menuID) === null) {
            $objectTypeFieldResolutionFeedbackStore->addError(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(MutationErrorFeedbackItemProvider::class, MutationErrorFeedbackItemProvider::E6, [$menuID ?? '']), $fieldDataAccessor->getField()));
        }
    }
    /**
     ...
     */
    protected function validateCanLoggedInUserDeleteMenu(string|int $menuID, FieldDataAccessorInterface $fieldDataAccessor, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : void
    {
        $userID = App::getState('current-user-id');
        if ($this->getMenuTypeMutationAPI()->canUserDeleteMenu($userID, $menuID)) {
            return;
        }
        $objectTypeFieldResolutionFeedbackStore->addError(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(MutationErrorFeedbackItemProvider::class, MutationErrorFeedbackItemProvider::E11, [$menuID]), $fieldDataAccessor->getField()));
    }
    /**
