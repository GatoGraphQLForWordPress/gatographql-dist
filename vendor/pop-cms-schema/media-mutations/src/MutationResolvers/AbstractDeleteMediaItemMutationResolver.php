...
{
    use \PoPCMSSchema\MediaMutations\MutationResolvers\DeleteMediaItemMutationResolverTrait;
    ...
    {
        return new FeedbackItemResolution(MutationErrorFeedbackItemProvider::class, MutationErrorFeedbackItemProvider::E10);
    }
     ...
     */
    protected function validateMediaItemExists(string|int|null $mediaItemID, FieldDataAccessorInterface $fieldDataAccessor, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : void
    {
        if (!$mediaItemID || !$this->getMediaTypeAPI()->mediaItemByIDExists($mediaItemID)) {
            $objectTypeFieldResolutionFeedbackStore->addError(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(MutationErrorFeedbackItemProvider::class, MutationErrorFeedbackItemProvider::E6, [$mediaItemID ?? '']), $fieldDataAccessor->getField()));
        }
    }
    /**
     ...
     */
    protected function validateCanLoggedInUserDeleteMediaItem(string|int $mediaItemID, FieldDataAccessorInterface $fieldDataAccessor, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : void
    {
        $userID = App::getState('current-user-id');
        if ($this->getMediaTypeMutationAPI()->canUserDeleteMediaItem($userID, $mediaItemID)) {
            return;
        }
        $objectTypeFieldResolutionFeedbackStore->addError(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(MutationErrorFeedbackItemProvider::class, MutationErrorFeedbackItemProvider::E11, [$mediaItemID]), $fieldDataAccessor->getField()));
    }
    /**
     * When not deleting permanently, the media item must support being
     * sent to the trash, and must not be in the trash already.
     */
    protected function validateCanMediaItemBeTrashed(string|int $mediaItemID, FieldDataAccessorInterface $fieldDataAccessor, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : void
    {
        if ($this->isForceDelete($fieldDataAccessor)) {
            return;
        }
        if (!$this->getMediaTypeMutationAPI()->doesMediaItemSupportTrash()) {
            $objectTypeFieldResolutionFeedbackStore->addError(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(MutationErrorFeedbackItemProvider::class, MutationErrorFeedbackItemProvider::E12, [$mediaItemID]), $fieldDataAccessor->getField()));
            return;
        }
        if ($this->getMediaTypeMutationAPI()->isMediaItemInTrash($mediaItemID)) {
            $objectTypeFieldResolutionFeedbackStore->addError(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(MutationErrorFeedbackItemProvider::class, MutationErrorFeedbackItemProvider::E13, [$mediaItemID]), $fieldDataAccessor->getField()));
        }
    }
    protected function isForceDelete(FieldDataAccessorInterface $fieldDataAccessor) : bool
    {
        return $fieldDataAccessor->getValue(MutationInputProperties::FORCE) === \true;
    }
    /**
