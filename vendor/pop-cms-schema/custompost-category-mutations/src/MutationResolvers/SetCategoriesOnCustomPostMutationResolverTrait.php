...
trait SetCategoriesOnCustomPostMutationResolverTrait
{
     ...
     */
    protected function validateCategoriesByIDExist(array $customPostCategoryIDs, FieldDataAccessorInterface $fieldDataAccessor, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : void
    {
     ...
     */
    protected function validateCategoriesBySlugExist(array $customPostCategorySlugs, FieldDataAccessorInterface $fieldDataAccessor, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : void
    {
        $query = ['slugs' => $customPostCategorySlugs];
        $existingCategorySlugs = $this->getCategoryTypeAPI()->getCategories($query, [QueryOptions::RETURN_TYPE => ReturnTypes::SLUGS]);
        $nonExistingCategorySlugs = \array_values(\array_diff($customPostCategorySlugs, $existingCategorySlugs));
        if ($nonExistingCategorySlugs !== []) {
            $objectTypeFieldResolutionFeedbackStore->addError(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(MutationErrorFeedbackItemProvider::class, MutationErrorFeedbackItemProvider::E3, [\implode($this->__('\', \'', 'custompost-category-mutations'), $nonExistingCategorySlugs)]), $fieldDataAccessor->getField()));
        }
    }
