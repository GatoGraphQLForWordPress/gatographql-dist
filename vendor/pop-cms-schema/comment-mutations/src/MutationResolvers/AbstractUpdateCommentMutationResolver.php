...
use stdClass;
/** @internal */
abstract class AbstractUpdateCommentMutationResolver extends \PoPCMSSchema\CommentMutations\MutationResolvers\AbstractEditCommentMutationResolver
{
    use \PoPCMSSchema\CommentMutations\MutationResolvers\UpdateCommentMutationResolverTrait;
    protected function validateUpdateErrors(FieldDataAccessorInterface $fieldDataAccessor, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : void
    {
        ...
        /** @var string|int */
        $commentID = $commentID;
        $this->validateCanLoggedInUserEditComment($commentID, MutationErrorFeedbackItemProvider::E11, $fieldDataAccessor, $objectTypeFieldResolutionFeedbackStore);
        if ($objectTypeFieldResolutionFeedbackStore->getErrorCount() > $errorCount) {
            return;
        }
        ...
        App::doAction(CommentCRUDHookNames::VALIDATE_UPDATE_COMMENT, $fieldDataAccessor, $objectTypeFieldResolutionFeedbackStore);
    }
    /**
     * The content is optional, but when provided it must not be empty.
     */
    protected function validateContentIsNotEmpty(FieldDataAccessorInterface $fieldDataAccessor, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : void
    {
        $content = $this->getContent($fieldDataAccessor);
        if ($content === null || \trim($content) !== '') {
            return;
        }
        $objectTypeFieldResolutionFeedbackStore->addError(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(MutationErrorFeedbackItemProvider::class, MutationErrorFeedbackItemProvider::E5), $fieldDataAccessor->getField()));
    }
    ...
    protected function getCommentData(FieldDataAccessorInterface $fieldDataAccessor) : array
    {
        ...
        foreach ([MutationInputProperties::AUTHOR_NAME => 'author', MutationInputProperties::AUTHOR_EMAIL => 'authorEmail', MutationInputProperties::AUTHOR_URL => 'authorURL'] as $inputFieldName => $commentDataKey) {
            if (!$fieldDataAccessor->hasValue($inputFieldName)) {
                continue;
            }
            $commentData[$commentDataKey] = $fieldDataAccessor->getValue($inputFieldName);
        }
        return App::applyFilters(CommentCRUDHookNames::GET_UPDATE_COMMENT_DATA, $commentData, $fieldDataAccessor);
    }
    ...
    protected function update(FieldDataAccessorInterface $fieldDataAccessor, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : string|int
    {
        ...
        if ($commentData !== []) {
            $this->getCommentTypeMutationAPI()->updateComment($commentID, $commentData);
        }
        /**
        ...
        if ($commentStatus !== null) {
            $this->getCommentTypeMutationAPI()->setCommentStatus($commentID, $commentStatus);
        }
        App::doAction(CommentCRUDHookNames::EXECUTE_UPDATE_COMMENT, $commentID, $fieldDataAccessor);
        return $commentID;
    }
}
