...
abstract class AbstractFieldDirectiveResolver extends \PoP\ComponentModel\DirectiveResolvers\AbstractDirectiveResolver implements \PoP\ComponentModel\DirectiveResolvers\FieldDirectiveResolverInterface
{
    ...
    public function prepareDirective(RelationalTypeResolverInterface $relationalTypeResolver, array $fields, EngineIterationFeedbackStore $engineIterationFeedbackStore) : void
    {
        ...
        if ($this->prepareDirectiveCacheTypeResolver === $relationalTypeResolver && $this->prepareDirectiveCacheFields === $fields) {
            return;
        }
        $directiveArgs = $this->getDirectiveArgs($relationalTypeResolver, $fields, $engineIterationFeedbackStore);
     ...
     */
    public function createDirectiveDataAccessor(array $directiveArgs) : DirectiveDataAccessorInterface
    {
        return new DirectiveDataAccessor($directiveArgs);
    }
    /**
     ...
     */
    protected function getDirectiveArgs(RelationalTypeResolverInterface $relationalTypeResolver, array $fields, EngineIterationFeedbackStore $engineIterationFeedbackStore) : ?array
    {
        ...
        $directiveArgs = $this->addDefaultFieldOrDirectiveArguments($directiveArgs, $directiveArgumentNameDefaultValues);
        /**
         * Cast the Arguments, return if any of them produced an error
         */
        $objectTypeFieldResolutionFeedbackStore = new ObjectTypeFieldResolutionFeedbackStore();
        $directiveArgs = $this->getSchemaCastingService()->castArguments($directiveArgs, $directiveArgsSchemaDefinition, $this->directive, $objectTypeFieldResolutionFeedbackStore);
        ...
        $this->validateDirectiveArgs($directiveArgs, $relationalTypeResolver, $fields, $engineIterationFeedbackStore);
     ...
     * @return array<string,mixed>|null Null if there was an error validating the directive
     */
    protected function getResolvedDirectiveArgs(RelationalTypeResolverInterface $relationalTypeResolver, array $idFieldSet, EngineIterationFeedbackStore $engineIterationFeedbackStore) : ?array
    {
        ...
        if ($hasArgumentReferencingPromise && $this->validatedDirectiveArgsHaveErrors === null) {
             ...
             */
            $errorCount = $engineIterationFeedbackStore->getErrorCount();
            $this->validateDirectiveArgs($directiveArgs, $relationalTypeResolver, MethodHelpers::getFieldsFromIDFieldSet($idFieldSet), $engineIterationFeedbackStore);
            if ($engineIterationFeedbackStore->getErrorCount() > $errorCount) {
                $this->validatedDirectiveArgsHaveErrors = \true;
                return null;
            }
    ...
    protected function getResolvedDirectiveArgsForObjectAndField(RelationalTypeResolverInterface $relationalTypeResolver, FieldInterface $field, string|int $id, EngineIterationFeedbackStore $engineIterationFeedbackStore) : ?array
    {
         ...
         */
        $errorCount = $engineIterationFeedbackStore->getErrorCount();
        $this->validateDirectiveArgs($directiveArgs, $relationalTypeResolver, [$field], $engineIterationFeedbackStore);
        if ($engineIterationFeedbackStore->getErrorCount() > $errorCount) {
            return null;
        }
        return $directiveArgs;
     ...
     * @param FieldInterface[] $fields
     */
    protected function validateDirectiveArgs(array $directiveArgs, RelationalTypeResolverInterface $relationalTypeResolver, array $fields, EngineIterationFeedbackStore $engineIterationFeedbackStore) : void
    {
        /** @var array<string,mixed> */
        $directiveArgsSchemaDefinition = $this->getDirectiveArgumentsSchemaDefinition($relationalTypeResolver);
        /**
         ...
         */
        $errorCount = $engineIterationFeedbackStore->getErrorCount();
        $this->validateNonMissingMandatoryDirectiveArguments($directiveArgs, $directiveArgsSchemaDefinition, $relationalTypeResolver, $fields, $engineIterationFeedbackStore);
        $this->validateOnlyExistingDirectiveArguments($directiveArgs, $directiveArgsSchemaDefinition, $relationalTypeResolver, $fields, $engineIterationFeedbackStore);
        if ($engineIterationFeedbackStore->getErrorCount() > $errorCount) {
            return;
        }
        /**
         ...
         */
        $this->validateDirectiveArgumentConstraints($directiveArgs, $relationalTypeResolver, $fields, $engineIterationFeedbackStore);
    }
    /**
     ...
     * @param array<string,mixed> $directiveArgs
     */
    private function validateOnlyExistingDirectiveArguments(array $directiveArgs, array $directiveArgsSchemaDefinition, RelationalTypeResolverInterface $relationalTypeResolver, array $fields, EngineIterationFeedbackStore $engineIterationFeedbackStore) : void
    {
        $nonExistingArgNames = \array_values(\array_diff(\array_keys($directiveArgs), \array_keys($directiveArgsSchemaDefinition)));
        foreach ($nonExistingArgNames as $nonExistingArgName) {
            $engineIterationFeedbackStore->schemaFeedbackStore->addError(new SchemaFeedback(new FeedbackItemResolution(GraphQLSpecErrorFeedbackItemProvider::class, GraphQLSpecErrorFeedbackItemProvider::E_5_4_1_B, [$this->directive->getName(), $nonExistingArgName]), $this->directive->getArgument($nonExistingArgName) ?? $this->directive, $relationalTypeResolver, $fields));
        }
    }
    /**
     ...
     * @param array<string,mixed> $directiveArgs
     */
    private function validateDirectiveArgumentConstraints(array $directiveArgs, RelationalTypeResolverInterface $relationalTypeResolver, array $fields, EngineIterationFeedbackStore $engineIterationFeedbackStore) : void
    {
        $objectTypeFieldResolutionFeedbackStore = new ObjectTypeFieldResolutionFeedbackStore();
     ...
     */
    protected final function getDirectiveArgumentNameDefaultValues(RelationalTypeResolverInterface $relationalTypeResolver) : ?array
    {
        $directiveArgsSchemaDefinition = $this->getDirectiveArgumentsSchemaDefinition($relationalTypeResolver);
     ...
     */
    protected final function getDirectiveArgumentsSchemaDefinition(RelationalTypeResolverInterface $relationalTypeResolver) : array
    {
    ...
    public function resolveCanProcessDirective(RelationalTypeResolverInterface $relationalTypeResolver, Directive $directive) : bool
    {
        ...
        if ($moduleConfiguration->enableSemanticVersionConstraints() && $this->hasDirectiveVersion($relationalTypeResolver)) {
             ...
             */
            $versionConstraint = $directive->getArgumentValue(SchemaDefinition::VERSION_CONSTRAINT) ?? $this->getVersioningService()->getVersionConstraintsForDirective($this) ?? App::getState('version-constraint');
            /**
    ...
    public final function resolveCanProcessField(RelationalTypeResolverInterface $relationalTypeResolver, FieldInterface $field) : bool
    {
        ...
        return $this->resolveCanProcessFieldBasedOnSupportedFieldTypeResolverClasses($objectTypeResolver, $field);
    }
    /**
     * Check if the directive only handles specific types
     */
    protected function resolveCanProcessFieldBasedOnSupportedFieldTypeResolverClasses(ObjectTypeResolverInterface $objectTypeResolver, FieldInterface $field) : bool
    {
        /**
         * The field type resolver may be provided via AppState or,
         * if absent, only then retrieve it from the Field.
         */
        $fieldTypeResolver = $this->getFieldTypeResolverFromAppStateOrField($objectTypeResolver, $field);
        /**
         * There will be a GraphQL error somewhere else,
         * process the field as to avoid adding yet another error
         */
     ...
     * if absent, only then retrieve it from the Field.
     */
    protected function getFieldTypeResolverFromAppStateOrField(ObjectTypeResolverInterface $objectTypeResolver, FieldInterface $field) : ?ConcreteTypeResolverInterface
    {
    ...
    protected function getSupportedConcreteTypeResolvers(array $supportedFieldTypeResolverContainerServiceIDs) : ?array
    {
        ...
        return \array_map(function (string $serviceID) : ConcreteTypeResolverInterface {
            /** @var ConcreteTypeResolverInterface */
    ...
    public final function hasDirectiveVersion(RelationalTypeResolverInterface $relationalTypeResolver) : bool
    {
    ...
    public function getConsolidatedDirectiveArgNameTypeResolvers(RelationalTypeResolverInterface $relationalTypeResolver) : array
    {
         ...
         */
        $consolidatedDirectiveArgNameTypeResolvers = App::applyFilters(\PoP\ComponentModel\DirectiveResolvers\HookNames::DIRECTIVE_ARG_NAME_TYPE_RESOLVERS, $directiveArgNameTypeResolvers, $this, $relationalTypeResolver);
        /**
    ...
    public function getConsolidatedDirectiveArgDescription(RelationalTypeResolverInterface $relationalTypeResolver, string $directiveArgName) : ?string
    {
        ...
        $this->consolidatedDirectiveArgDescriptionCache[$cacheKey] = App::applyFilters(\PoP\ComponentModel\DirectiveResolvers\HookNames::DIRECTIVE_ARG_DESCRIPTION, $this->getDirectiveArgDescription($relationalTypeResolver, $directiveArgName), $this, $relationalTypeResolver, $directiveArgName);
        return $this->consolidatedDirectiveArgDescriptionCache[$cacheKey];
    }
    /**
     * Consolidation of the schema directive arguments. Call this function to read the data
    ...
    public function getConsolidatedDirectiveArgDefaultValue(RelationalTypeResolverInterface $relationalTypeResolver, string $directiveArgName) : mixed
    {
        ...
        $this->consolidatedDirectiveArgDefaultValueCache[$cacheKey] = App::applyFilters(\PoP\ComponentModel\DirectiveResolvers\HookNames::DIRECTIVE_ARG_DEFAULT_VALUE, $this->getDirectiveArgDefaultValue($relationalTypeResolver, $directiveArgName), $this, $relationalTypeResolver, $directiveArgName);
        return $this->consolidatedDirectiveArgDefaultValueCache[$cacheKey];
    }
    /**
     * Consolidation of the schema directive arguments. Call this function to read the data
    ...
    public function getConsolidatedDirectiveArgTypeModifiers(RelationalTypeResolverInterface $relationalTypeResolver, string $directiveArgName) : int
    {
        ...
        $this->consolidatedDirectiveArgTypeModifiersCache[$cacheKey] = App::applyFilters(\PoP\ComponentModel\DirectiveResolvers\HookNames::DIRECTIVE_ARG_TYPE_MODIFIERS, $this->getDirectiveArgTypeModifiers($relationalTypeResolver, $directiveArgName), $this, $relationalTypeResolver, $directiveArgName);
        return $this->consolidatedDirectiveArgTypeModifiersCache[$cacheKey];
    }
    /**
     * Consolidation of the schema directive arguments. Call this function to read the data
    ...
    public final function getDirectiveArgsSchemaDefinition(RelationalTypeResolverInterface $relationalTypeResolver) : array
    {
        ...
        foreach ($consolidatedDirectiveArgNameTypeResolvers as $directiveArgName => $directiveArgInputTypeResolver) {
            $directiveArgDescription = $this->getConsolidatedDirectiveArgDescription($relationalTypeResolver, $directiveArgName) ?? $directiveArgInputTypeResolver->getTypeDescription();
            $schemaDirectiveArgs[$directiveArgName] = $this->getFieldOrDirectiveArgTypeSchemaDefinition($directiveArgName, $directiveArgInputTypeResolver, $directiveArgDescription, $this->getConsolidatedDirectiveArgDefaultValue($relationalTypeResolver, $directiveArgName), $this->getConsolidatedDirectiveArgTypeModifiers($relationalTypeResolver, $directiveArgName));
    ...
    protected function getDirectiveArgExtensionsSchemaDefinition(RelationalTypeResolverInterface $relationalTypeResolver, string $directiveArgName) : array
    {
        ...
        return [SchemaDefinition::IS_SENSITIVE_DATA_ELEMENT => \false];
    }
    /**
     * Consolidation of the schema directive arguments. Call this function to read the data
     * instead of the individual functions, since it applies hooks to override/extend.
    ...
    protected final function getConsolidatedDirectiveArgExtensionsSchemaDefinition(RelationalTypeResolverInterface $relationalTypeResolver, string $directiveArgName) : array
    {
        ...
        $this->consolidatedDirectiveArgExtensionsCache[$cacheKey] = App::applyFilters(\PoP\ComponentModel\DirectiveResolvers\HookNames::DIRECTIVE_ARG_EXTENSIONS, $this->getDirectiveArgExtensionsSchemaDefinition($relationalTypeResolver, $directiveArgName), $this, $relationalTypeResolver, $directiveArgName);
        return $this->consolidatedDirectiveArgExtensionsCache[$cacheKey];
    }
    ...
    public function resolveDirectivePipelinePayload(array $payload) : array
    {
        ...
        list($relationalTypeResolver, $pipelineFieldDirectiveResolvers, $idObjects, $unionTypeOutputKeyIDs, $previouslyResolvedIDFieldValues, $pipelineIDFieldSet, $pipelineFieldDataAccessProviders, $resolvedIDFieldValues, $messages, $engineIterationFeedbackStore, ) = DirectivePipelineUtils::extractArgumentsFromPayload($payload);
        ...
        if (!$this->needsSomeIDFieldToExecute() || $this->hasSomeIDField($idFieldSet)) {
            ...
            $astNode = null;
            try {
                $this->resolveDirective($relationalTypeResolver, $idFieldSet, $directiveDataAccessProvider, $pipelineFieldDirectiveResolvers, $idObjects, $unionTypeOutputKeyIDs, $previouslyResolvedIDFieldValues, $pipelineIDFieldSet, $pipelineFieldDataAccessProviders, $resolvedIDFieldValues, $messages, $engineIterationFeedbackStore);
                ...
                $feedbackItemResolution = new FeedbackItemResolution(GenericFeedbackItemProvider::class, GenericFeedbackItemProvider::E1, [$e->getMessage()]);
            } catch (Exception $e) {
                /** @var ModuleConfiguration */
                $moduleConfiguration = App::getModule(Module::class)->getConfiguration();
                if ($moduleConfiguration->logExceptionErrorMessagesAndTraces()) {
                    $engineIterationFeedbackStore->objectResolutionFeedbackStore->addLog(new ObjectResolutionFeedback(new FeedbackItemResolution(ErrorFeedbackItemProvider::class, ErrorFeedbackItemProvider::E11A, [$this->directive->asQueryString(), $e->getMessage(), $e->getTraceAsString()]), $this->directive, $relationalTypeResolver, $this->directive, $idFieldSet));
                }
                $feedbackItemResolution = $moduleConfiguration->sendExceptionErrorMessages() ? $moduleConfiguration->sendExceptionTraces() ? new FeedbackItemResolution(ErrorFeedbackItemProvider::class, ErrorFeedbackItemProvider::E11A, [$this->directive->asQueryString(), $e->getMessage(), $e->getTraceAsString()]) : new FeedbackItemResolution(ErrorFeedbackItemProvider::class, ErrorFeedbackItemProvider::E11, [$this->directive->asQueryString(), $e->getMessage()]) : new FeedbackItemResolution(ErrorFeedbackItemProvider::class, ErrorFeedbackItemProvider::E12, [$this->directive->asQueryString()]);
            }
            if ($feedbackItemResolution !== null) {
                $this->processObjectFailure($relationalTypeResolver, $feedbackItemResolution, $idFieldSet, $pipelineIDFieldSet, $astNode ?? $this->directive, $resolvedIDFieldValues, $engineIterationFeedbackStore);
            }
        }
        // 3. Re-create the payload from the modified variables
        return DirectivePipelineUtils::convertArgumentsToPayload($relationalTypeResolver, $pipelineFieldDirectiveResolvers, $idObjects, $unionTypeOutputKeyIDs, $previouslyResolvedIDFieldValues, $pipelineIDFieldSet, $pipelineFieldDataAccessProviders, $resolvedIDFieldValues, $messages, $engineIterationFeedbackStore);
    }
    /**
     ...
     */
    protected function processObjectFailure(RelationalTypeResolverInterface $relationalTypeResolver, FeedbackItemResolution $feedbackItemResolution, array $idFieldSetToRemove, array &$succeedingPipelineIDFieldSet, AstInterface $astNode, array &$resolvedIDFieldValues, EngineIterationFeedbackStore $engineIterationFeedbackStore) : void
    {
        $this->processFailure($relationalTypeResolver, $feedbackItemResolution, $idFieldSetToRemove, $succeedingPipelineIDFieldSet, $astNode, $resolvedIDFieldValues, $engineIterationFeedbackStore->objectResolutionFeedbackStore);
    }
    /**
     ...
     * @param array<string|int,SplObjectStorage<FieldInterface,mixed>> $resolvedIDFieldValues
     */
    protected function processSchemaFailure(RelationalTypeResolverInterface $relationalTypeResolver, FeedbackItemResolution $feedbackItemResolution, array $idFieldSetToRemove, array &$succeedingPipelineIDFieldSet, AstInterface $astNode, array &$resolvedIDFieldValues, EngineIterationFeedbackStore $engineIterationFeedbackStore) : void
    {
        $this->processFailure($relationalTypeResolver, $feedbackItemResolution, $idFieldSetToRemove, $succeedingPipelineIDFieldSet, $astNode, $resolvedIDFieldValues, $engineIterationFeedbackStore->schemaFeedbackStore);
    }
    /**
     * @param array<string|int,SplObjectStorage<FieldInterface,mixed>> $resolvedIDFieldValues
     * @param array<string|int,EngineIterationFieldSet> $idFieldSet
     */
    protected function setFieldResponseValueAsNull(array &$resolvedIDFieldValues, array $idFieldSet) : void
    {
        foreach ($idFieldSet as $id => $fieldSet) {
     ...
     */
    protected function getSchemaDefinitionResolver(RelationalTypeResolverInterface $relationalTypeResolver) : \PoP\ComponentModel\DirectiveResolvers\SchemaFieldDirectiveResolverInterface
    {
        return $this;
    }
    /**
    ...
    public function skipExposingDirectiveInSchema(RelationalTypeResolverInterface $relationalTypeResolver) : bool
    {
        ...
        if ($moduleConfiguration->skipExposingDangerouslyNonSpecificScalarTypeTypeInSchema()) {
            ...
            if ($this->hasMandatoryDangerouslyNonSpecificScalarTypeInputType($consolidatedDirectiveArgNameTypeResolvers, $consolidatedDirectiveArgsTypeModifiers)) {
                return \true;
            }
        }
        return \false;
     ...
     * @return array<string,mixed>
     */
    public final function getDirectiveSchemaDefinition(RelationalTypeResolverInterface $relationalTypeResolver) : array
    {
    ...
    private function doGetDirectiveSchemaDefinition(RelationalTypeResolverInterface $relationalTypeResolver) : array
    {
        ...
        $schemaDefinition = [SchemaDefinition::NAME => $directiveName, SchemaDefinition::DIRECTIVE_KIND => $this->getDirectiveKind(), SchemaDefinition::DIRECTIVE_LOCATIONS => $this->getDirectiveLocations(), SchemaDefinition::DIRECTIVE_IS_REPEATABLE => $this->isRepeatable(), SchemaDefinition::DIRECTIVE_IS_GLOBAL => $this->isGlobal($relationalTypeResolver)];
        ...
        if ($deprecationMessage = $this->getDirectiveDeprecationMessage($relationalTypeResolver)) {
            $schemaDefinition[SchemaDefinition::DEPRECATED] = \true;
    ...
    public function getDirectiveLocations() : array
    {
         ...
         */
        if (\in_array($fieldDirectiveBehavior, [FieldDirectiveBehaviors::OPERATION, FieldDirectiveBehaviors::FIELD_AND_OPERATION])) {
            if ($isQueryTypeDirective) {
                $directiveLocations = [DirectiveLocations::QUERY, DirectiveLocations::MUTATION];
            }
        }
        /**
         * Add the "Field" Directive Locations
         */
        if (\in_array($fieldDirectiveBehavior, [FieldDirectiveBehaviors::FIELD, FieldDirectiveBehaviors::FIELD_AND_OPERATION])) {
            if ($isQueryTypeDirective) {
                /**
    ...
    protected function isQueryTypeDirective() : bool
    {
        ...
        $directiveKind = $this->getDirectiveKind();
        return $directiveKind === DirectiveKinds::QUERY || $directiveKind === DirectiveKinds::SCHEMA && $componentModelModuleConfiguration->includeSchemaTypeDirectivesInSchema() || $directiveKind === DirectiveKinds::INDEXING && $graphQLParserModuleConfiguration->enableComposableDirectives();
    }
    /**
