...
class Engine extends AbstractBasicService implements \PoP\ComponentModel\Engine\EngineInterface
{
    ...
    protected function getEtagHeader() : ?string
    {
        ...
        $differentiators = array($moduleInfo->getUniqueID(), (string) $moduleInfo->getRand(), (string) $moduleInfo->getTime());
    ...
    protected function generateData() : void
    {
        ...
        $engineState->schemaFeedbackEntries = $engineState->objectFeedbackEntries = [FeedbackCategories::ERROR => [], FeedbackCategories::WARNING => [], FeedbackCategories::DEPRECATION => [], FeedbackCategories::NOTICE => [], FeedbackCategories::SUGGESTION => [], FeedbackCategories::LOG => []];
        ...
        /** @var mixed[]|null */
        $sequentialMQEOperations = App::applyFilters(\PoP\ComponentModel\Engine\EngineHookNames::MULTIPLE_QUERY_EXECUTION_SEQUENTIAL_OPERATIONS, null);
        ...
        if (\in_array(DataOutputItems::DATABASES, $dataoutputitems)) {
            $engineState = App::getEngineState();
            $engineState->data = \array_replace_recursive($engineState->data, $this->formatAccumulatedDatabasesForOutput($engineState->databases, $engineState->unionTypeOutputKeyIDs));
        }
    ...
    protected function processAndGenerateData() : void
    {
        ...
        if ($needToComputeProps) {
            $appStateManager = App::getAppStateManager();
            $savedMQEOperation = $appStateManager->has('multiple-query-execution-current-operation') ? $appStateManager->get('multiple-query-execution-current-operation') : null;
            try {
                $appStateManager->override('multiple-query-execution-current-operation', null);
            ...
            $engineState->propsHaveBeenComputed = \true;
        ...
        if (\in_array(DataOutputItems::COMPONENT_DATA, $dataoutputitems) || \in_array(DataOutputItems::DATABASES, $dataoutputitems)) {
            ...
            if (\in_array(DataOutputItems::DATABASES, $dataoutputitems)) {
                ...
                $this->generateDatabases($schemaFeedbackEntries, $objectFeedbackEntries, $engineState->databases, $engineState->unionTypeOutputKeyIDs, $engineState->combinedUnionTypeOutputKeyIDs, $engineState->previouslyResolvedIDFieldValues, $engineState->messages);
    ...
    public function getRequestMeta() : array
    {
        ...
        if ($not_excluded_component_sets !== null) {
            ...
            foreach ($not_excluded_component_sets as $components) {
                $filteredsettings[] = \array_map($componentHelpers->getComponentOutputName(...), $components);
            }
    ...
    {
        $relationalTypeOutputKeyIDFieldSets[$relationalTypeOutputKey] ??= [self::DATA_PROP_RELATIONAL_TYPE_RESOLVER => $relationalTypeResolver, self::DATA_PROP_ID_FIELD_SET => []];
        foreach ($ids as $id) {
            /** @var EngineIterationFieldSet */
            $engineIterationFieldSet = $relationalTypeOutputKeyIDFieldSets[$relationalTypeOutputKey][self::DATA_PROP_ID_FIELD_SET][$id] ?? new EngineIterationFieldSet(\array_map(fn(ComponentFieldNodeInterface $componentFieldNode) => $componentFieldNode->getField(), $this->getDBObjectMandatoryFields()));
            // Add the 'direct' fields
            $engineIterationFieldSet->addFields(\array_map(fn(ComponentFieldNodeInterface $componentFieldNode) => $componentFieldNode->getField(), $directComponentFieldNodes));
            /**
            ...
            foreach ($conditionalComponentFieldNodesSplObjectStorage as $conditionComponentFieldNode) {
                ...
                $engineIterationFieldSet->conditionalFields[$conditionField] = \array_merge($engineIterationFieldSet->conditionalFields[$conditionField] ??= [], \array_map(fn(ComponentFieldNodeInterface $componentFieldNode) => $componentFieldNode->getField(), $conditionalComponentFieldNodes));
            }
     ...
     * @param Component[] $component_path
     */
    protected function assignValueForComponent(array &$array, array $component_path, Component $component, string $key, mixed $value) : void
    {
        /** @var ModuleInfo */
        $moduleInfo = App::getModule(Module::class)->getInfo();
     ...
     */
    protected function generateDatabases(array &$schemaFeedbackEntries, array &$objectFeedbackEntries, array &$databases, array &$unionTypeOutputKeyIDs, array &$combinedUnionTypeOutputKeyIDs, array &$previouslyResolvedIDFieldValues, array &$messages) : void
    {
        $engineState = App::getEngineState();
                ...
                if ($subcomponents_data_properties = $typeResolver_data[DataProperties::SUBCOMPONENTS]) {
                    ...
                    if ($isUnionTypeResolver) {
                        ...
                        foreach ($objectTypeResolver_ids as $id) {
                            ...
                            $iterationObjectTypeResolverNameDataItems[$objectTypeResolverName] ??= ['targetObjectTypeResolver' => $targetObjectTypeResolver, 'objectIDs' => []];
     ...
     * @return array<string,mixed>
     */
    protected function formatAccumulatedDatabasesForOutput(array $databases, array $unionTypeOutputKeyIDs) : array
    {
        $ret = [];
    ...
    protected function processSubcomponentData(RelationalTypeResolverInterface $relationalTypeResolver, ObjectTypeResolverInterface $targetObjectTypeResolver, array $typeResolverIDs, string $component_path_key, array &$databases, SplObjectStorage $subcomponents_data_properties, array &$already_loaded_id_fields, array &$unionTypeOutputKeyIDs, array &$combinedUnionTypeOutputKeyIDs) : void
    {
        ...
        foreach ($subcomponents_data_properties as $componentFieldNode) {
            ...
            $subcomponent_direct_fields = self::dedupComponentFieldNodes($subcomponent_data_properties[DataProperties::DIRECT_COMPONENT_FIELD_NODES] ?? []);
            ...
            if ($subcomponent_direct_fields || $subcomponent_conditional_fields_storage->count() > 0) {
                    ...
                    foreach ($databases as $dbName => $database) {
                        ...
                        foreach ($idsList as $database_field_id) {
                            if ($database_field_id !== null) {
                                $allSubcomponentIDsSet[$database_field_id] = \true;
                            }
                        }
                    ...
                    if ($subcomponent_already_loaded_id_fields === []) {
                        ...
                        foreach ($field_ids as $field_id) {
                            ...
                            foreach ($subcomponent_already_loaded_data_fields as $alreadyLoadedField) {
                                $alreadyLoadedFieldUniqueIDs[$alreadyLoadedField->getUniqueID()] = \true;
                            }
                ...
                if ($engineState->dbdata[$subcomponentTypeOutputKey][$component_path_key] ?? null) {
                    ...
                    $engineState->dbdata[$subcomponentTypeOutputKey][$component_path_key][DataProperties::DIRECT_COMPONENT_FIELD_NODES] = self::dedupComponentFieldNodes($engineState->dbdata[$subcomponentTypeOutputKey][$component_path_key][DataProperties::DIRECT_COMPONENT_FIELD_NODES]);
                }
            }
        }
    }
        ...
        if ($subcomponents_data_properties = $data_properties[DataProperties::SUBCOMPONENTS] ?? null) {
            ...
            foreach ($subcomponents_data_properties as $componentFieldNode) {
                ...
                if (isset($componentFieldNodeData[DataProperties::DIRECT_COMPONENT_FIELD_NODES])) {
                    $dbDataSubcomponentsFieldSplObjectStorage[DataProperties::DIRECT_COMPONENT_FIELD_NODES] = self::dedupComponentFieldNodes(\array_merge($dbDataSubcomponentsFieldSplObjectStorage[DataProperties::DIRECT_COMPONENT_FIELD_NODES] ?? [], $componentFieldNodeData[DataProperties::DIRECT_COMPONENT_FIELD_NODES]));
                }
