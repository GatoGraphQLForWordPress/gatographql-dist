...
class VersioningService extends AbstractBasicService implements \PoP\ComponentModel\Versioning\VersioningServiceInterface
{
    ...
    protected function initializeVersionConstraintsForFields() : void
    {
        ...
        foreach (App::getState('field-version-constraints') ?? [] as $typeField => $versionConstraint) {
            ...
            if (\count($entry) !== 2) {
                $generalFeedbackStore->addWarning(new GeneralFeedback(new FeedbackItemResolution(WarningFeedbackItemProvider::class, WarningFeedbackItemProvider::W1, [$typeField])));
                continue;
            }
     ...
     */
    public function getVersionConstraintsForField(ObjectTypeResolverInterface $objectTypeResolver, FieldInterface $field) : ?string
    {
        if ($this->versionConstraintsForFields === null) {
            $this->initializeVersionConstraintsForFields();
        }
        $fieldName = $field->getName();
        return $this->versionConstraintsForFields[$objectTypeResolver->getNamespacedTypeName()][$fieldName] ?? $this->versionConstraintsForFields[$objectTypeResolver->getTypeName()][$fieldName] ?? $this->versionConstraintsForFields[ConfigurationValues::ANY][$fieldName] ?? null;
    }
    /**
