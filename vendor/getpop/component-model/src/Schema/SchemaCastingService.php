...
class SchemaCastingService implements \PoP\ComponentModel\Schema\SchemaCastingServiceInterface
{
    ...
    protected final function getInputCoercingService() : \PoP\ComponentModel\Schema\InputCoercingServiceInterface
    {
        if ($this->inputCoercingService === null) {
            ...
            $inputCoercingService = $this->instanceManager->getInstance(\PoP\ComponentModel\Schema\InputCoercingServiceInterface::class);
            $this->inputCoercingService = $inputCoercingService;
        }
    ...
    public function castArguments(array $argumentKeyValues, array $argumentSchemaDefinition, WithArgumentsInterface $withArgumentsAST, ObjectTypeFieldResolutionFeedbackStore $objectTypeFieldResolutionFeedbackStore) : array
    {
        ...
        foreach ($argumentKeyValues as $argName => $argValue) {
            ...
            $fieldOrDirectiveArgTypeResolver = $argumentSchemaDefinition[$argName][\PoP\ComponentModel\Schema\SchemaDefinition::TYPE_RESOLVER];
            /**
             ...
             **/
            $fieldOrDirectiveArgIsNonNullable = $argumentSchemaDefinition[$argName][\PoP\ComponentModel\Schema\SchemaDefinition::NON_NULLABLE] ?? \false;
            $fieldOrDirectiveArgIsArrayType = $argumentSchemaDefinition[$argName][\PoP\ComponentModel\Schema\SchemaDefinition::IS_ARRAY] ?? \false;
            $fieldOrDirectiveArgIsNonNullArrayItemsType = $argumentSchemaDefinition[$argName][\PoP\ComponentModel\Schema\SchemaDefinition::IS_NON_NULLABLE_ITEMS_IN_ARRAY] ?? \false;
            $fieldOrDirectiveArgIsArrayOfArraysType = $argumentSchemaDefinition[$argName][\PoP\ComponentModel\Schema\SchemaDefinition::IS_ARRAY_OF_ARRAYS] ?? \false;
            $fieldOrDirectiveArgIsNonNullArrayOfArraysItemsType = $argumentSchemaDefinition[$argName][\PoP\ComponentModel\Schema\SchemaDefinition::IS_NON_NULLABLE_ITEMS_IN_ARRAY_OF_ARRAYS] ?? \false;
            /**
            ...
            if ($isDangerouslyNonSpecificScalar) {
                 ...
                 */
                $argValue = $inputCoercingService->maybeConvertInputValueFromSingleToList($argValue, $fieldOrDirectiveArgIsNonNullable, $fieldOrDirectiveArgIsArrayType, $fieldOrDirectiveArgIsArrayOfArraysType);
            }
            ...
            $inputCoercingService->validateInputArrayModifiers($fieldOrDirectiveArgTypeResolver, $argValue, $argName, $fieldOrDirectiveArgIsNonNullable, $fieldOrDirectiveArgIsArrayType, $fieldOrDirectiveArgIsNonNullArrayItemsType, $fieldOrDirectiveArgIsArrayOfArraysType, $fieldOrDirectiveArgIsNonNullArrayOfArraysItemsType, $astNode, $objectTypeFieldResolutionFeedbackStore);
             ...
             */
            $coercedArgValue = $inputCoercingService->coerceInputValue($fieldOrDirectiveArgTypeResolver, $argValue, $argName, $fieldOrDirectiveArgIsNonNullable, $fieldOrDirectiveArgIsArrayType, $fieldOrDirectiveArgIsArrayOfArraysType, $astNode, $objectTypeFieldResolutionFeedbackStore);
            if ($objectTypeFieldResolutionFeedbackStore->getErrorCount() > $errorCount) {
                continue;
            }
            /**
            ...
            if ($fieldOrDirectiveArgTypeResolver instanceof DeprecatableInputTypeResolverInterface) {
                $deprecationMessages = $inputCoercingService->getInputValueDeprecationMessages($fieldOrDirectiveArgTypeResolver, $coercedArgValue, $fieldOrDirectiveArgIsArrayType, $fieldOrDirectiveArgIsArrayOfArraysType);
                foreach ($deprecationMessages as $deprecationMessage) {
                    $objectTypeFieldResolutionFeedbackStore->addDeprecation(new ObjectTypeFieldResolutionFeedback(new FeedbackItemResolution(GenericFeedbackItemProvider::class, GenericFeedbackItemProvider::D1, [$deprecationMessage]), $astNode));
                }
            }
        }
