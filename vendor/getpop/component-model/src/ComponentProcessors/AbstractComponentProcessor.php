...
abstract class AbstractComponentProcessor extends AbstractBasicService implements \PoP\ComponentModel\ComponentProcessors\ComponentProcessorInterface
{
    ...
    protected final function getComponentProcessorManager() : \PoP\ComponentModel\ComponentProcessors\ComponentProcessorManagerInterface
    {
        if ($this->componentProcessorManager === null) {
            ...
            $componentProcessorManager = $this->instanceManager->getInstance(\PoP\ComponentModel\ComponentProcessors\ComponentProcessorManagerInterface::class);
     ...
     * @return Component[]
     */
    public final function getAllSubcomponents(Component $component) : array
    {
    ...
    private function getSubcomponentCacheKeyForComponent(Component $component) : string
    {
        ...
        if ($this->cacheForExecutableDocumentAST !== $executableDocument || $this->cacheForMultipleQueryExecutionCurrentOperation !== $currentMQEOperation) {
    ...
    public function executeInitPropsComponentTree(callable $eval_self_fn, callable $get_props_for_descendant_components_fn, callable $get_props_for_descendant_datasetcomponents_fn, string $propagate_fn, Component $component, array &$props, array $wildcard_props_to_propagate, array $targeted_props_to_propagate) : void
    {
        ...
        if ($targetedPropsForComponent === []) {
            ...
            $props[$componentFullName] = \array_merge_recursive($targetedPropsForComponent, $existingPropsForComponent);
        }
        ...
        if ($descendantWildcardProps !== []) {
            $wildcard_props_to_propagate = \array_merge($wildcard_props_to_propagate, $descendantWildcardProps);
        }
        // Propagate
            ...
            foreach ($subcomponents as $subcomponent) {
                ...
                if (!$subcomponent_processor->startDataloadingSection($subcomponent)) {
                    ...
                    if ($datasetDescendantProps !== []) {
                        $subcomponent_wildcard_props_to_propagate = \array_merge($subcomponent_wildcard_props_to_propagate, $datasetDescendantProps);
                    }
                }
    ...
    public function addToDatasetOutputKeysWithPrefix(Component $component, array &$props, string $pathPrefix, array &$ret) : void
    {
        ...
        if ($this->getProp($component, $props, 'succeeding-typeResolver') !== null) {
            ...
            foreach ($this->getRelationalComponentFieldNodes($component) as $relationalComponentFieldNode) {
                $subPathPrefix = $pathPrefix === '' ? $relationalComponentFieldNode->getField()->getOutputKey() : $pathPrefix . Constants::RELATIONAL_FIELD_PATH_SEPARATOR . $relationalComponentFieldNode->getField()->getOutputKey();
                foreach ($relationalComponentFieldNode->getNestedComponents() as $subcomponent_component) {
    ...
    {
        $pathPrefixWithSep = $pathPrefix === '' ? '' : $pathPrefix . Constants::RELATIONAL_FIELD_PATH_SEPARATOR;
        if ($relationalTypeResolver = $this->getRelationalTypeResolver($component)) {
            /**
        ...
        if ($relationalTypeResolver = $this->getProp($component, $props, 'succeeding-typeResolver')) {
            ...
            foreach ($this->getRelationalComponentFieldNodes($component) as $relationalComponentFieldNode) {
                ...
                $typeResolver = $dataloadHelperService->getTypeResolverFromSubcomponentField($relationalTypeResolver, $relationalField);
    ...
    public function getComponentsToPropagateDataProperties(Component $component) : array
    {
        ...
        return $this->componentsToPropagateDataPropertiesCache[$cacheKey] = $this->getSubcomponentsByGroup($component, array(self::COMPONENTELEMENT_SUBCOMPONENTS, self::COMPONENTELEMENT_CONDITIONALONDATAFIELDSUBCOMPONENTS));
    }
    /**
     * @param array<string,mixed> $ret
     * @param array<string,mixed> $props
        ...
        if ($subcomponents = $this->getComponentsToPropagateDataProperties($component)) {
            ...
            if ($conditionalLeafComponentFieldNodes !== [] || $conditionalRelationalComponentFieldNodes !== []) {
                ...
                foreach ($directSubcomponents as $directSubcomponent) {
                    $directSubcomponentKeys[$componentKey($directSubcomponent)] = \true;
                }
                /**
                ...
                foreach ($conditionalComponentFieldNodes as $conditionComponentFieldNode) {
                    ...
                    $conditionalSubcomponents = \array_values(\array_filter($conditionalSubcomponents, static fn(Component $c): bool => !isset($directSubcomponentKeys[$componentKey($c)])));
                    ...
                    foreach ($conditionalSubcomponents as $conditionalSubcomponent) {
                        $conditionalSubcomponentKeysToRemoveFromSubcomponents[$componentKey($conditionalSubcomponent)] = \true;
                    }
                }
                if ($conditionalSubcomponentKeysToRemoveFromSubcomponents !== []) {
                    $subcomponents = \array_values(\array_filter($subcomponents, static fn(Component $c): bool => !isset($conditionalSubcomponentKeysToRemoveFromSubcomponents[$componentKey($c)])));
                }
            }
    ...
    protected function flattenRelationalDBObjectDataProperties(string $propagate_fn, array &$ret, Component $component, array &$props) : void
    {
        ...
        foreach ($relationalSubcomponents as $subcomponentComponentFieldNode) {
            ...
            if ($subcomponent_components_data_properties[DataProperties::DIRECT_COMPONENT_FIELD_NODES]) {
                $subcomponentsSubcomponentFieldNode[DataProperties::DIRECT_COMPONENT_FIELD_NODES] = self::dedupComponentFieldNodes(\array_merge($subcomponentsSubcomponentFieldNode[DataProperties::DIRECT_COMPONENT_FIELD_NODES] ?? [], $subcomponent_components_data_properties[DataProperties::DIRECT_COMPONENT_FIELD_NODES]));
            }
            /** @var SplObjectStorage<ComponentFieldNodeInterface,ComponentFieldNodeInterface[]> */
            // @phpstan-ignore-next-line
