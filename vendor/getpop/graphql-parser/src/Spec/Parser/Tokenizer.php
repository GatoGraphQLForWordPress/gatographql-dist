...
class Tokenizer
{
     ...
     * @throws SyntaxErrorParserException
     */
    protected function expect(string $type) : \PoP\GraphQLParser\Spec\Parser\Token
    {
        if ($this->match($type)) {
            return $this->lex();
        }
     ...
     */
    protected function scanString(bool $isBlockString) : \PoP\GraphQLParser\Spec\Parser\Token
    {
        $sourceLength = \strlen($this->source);
        $this->pos++;
        $value = '';
    ...
    protected function createUnexpectedException(\PoP\GraphQLParser\Spec\Parser\Token $token) : SyntaxErrorParserException
    {
