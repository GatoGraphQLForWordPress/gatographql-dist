...
class AppStateManager implements \PoP\Root\StateManagers\AppStateManagerInterface
{
    ...
    public function initializeAppState(array $initialAppState) : void
    {
        ...
        $this->state = App::applyFilters(HookNames::APP_STATE_BEFORE_INITIALIZED, $this->state);
         ...
         */
        $this->state = \array_merge($this->state, $initialAppState);
        // Allow to inject state (eg: during tests on DEV)
        $this->state = App::applyFilters(HookNames::APP_STATE_INITIALIZED, $this->state);
        // Second pass: consolidate
        foreach ($appStateProviders as $appStateProvider) {
            $appStateProvider->consolidate($this->state);
        }
        // Allow to inject state (eg: during tests on DEV)
        $this->state = App::applyFilters(HookNames::APP_STATE_CONSOLIDATED, $this->state);
        // Third pass: augment
        foreach ($appStateProviders as $appStateProvider) {
            $appStateProvider->augment($this->state);
        }
        // Allow to inject state (eg: during tests on DEV)
        $this->state = App::applyFilters(HookNames::APP_STATE_AUGMENTED, $this->state);
        // Final pass: compute
        foreach ($appStateProviders as $appStateProvider) {
            $appStateProvider->compute($this->state);
        }
        // Allow to inject state (eg: during tests on DEV)
        $this->state = App::applyFilters(HookNames::APP_STATE_COMPUTED, $this->state);
    ...
    public function executeAppState() : void
    {
        ...
        // Allow to inject state (eg: during tests on DEV)
        $this->state = App::applyFilters(HookNames::APP_STATE_EXECUTED, $this->state);
    }
    /**
