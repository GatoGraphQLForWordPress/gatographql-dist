...
final class DebugCommand extends Command
{
    ...
    {
        $this->setDefinition([new InputArgument('filter', InputArgument::OPTIONAL, 'The name of an environment variable or a filter.', null, $this->getAvailableVars(...))])->setHelp(<<<'EOT'
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        $io->listing(\array_map(fn(string $envFile) => \in_array($envFile, $availableFiles, \true) ? \sprintf('<fg=green>✓</> %s', $this->getRelativeName($envFile)) : \sprintf('<fg=red>⨯</> %s', $this->getRelativeName($envFile)), $envFiles));
        ...
        if ($variables || null === $nameFilter) {
            $io->table(\array_merge(['Variable', 'Value'], \array_map($this->getRelativeName(...), $availableFiles)), $variables);
    ...
    {
        $files = [\sprintf('%s.local.php', $filePath), \sprintf('%s.%s.local', $filePath, $this->kernelEnvironment), \sprintf('%s.%s', $filePath, $this->kernelEnvironment)];
