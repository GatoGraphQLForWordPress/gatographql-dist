...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        if (\in_array('.env.local.php', $availableFiles, \true)) {
        ...
        $io->listing(\array_map(static fn(string $envFile) => \in_array($envFile, $availableFiles, \true) ? \sprintf('<fg=green>✓</> %s', $envFile) : \sprintf('<fg=red>⨯</> %s', $envFile), $envFiles));
        ...
        if ($variables || null === $nameFilter) {
            $io->table(\array_merge(['Variable', 'Value'], $availableFiles), $this->getVariables($availableFiles, $nameFilter));
    ...
    {
        $files = ['.env.local.php', \sprintf('.env.%s.local', $this->kernelEnvironment), \sprintf('.env.%s', $this->kernelEnvironment)];
