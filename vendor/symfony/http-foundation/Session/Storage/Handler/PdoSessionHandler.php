...
use GatoExternalPrefixByGatoGraphQL\Doctrine\DBAL\Schema\Column;
use GatoExternalPrefixByGatoGraphQL\Doctrine\DBAL\Schema\Index;
use GatoExternalPrefixByGatoGraphQL\Doctrine\DBAL\Schema\Name\Identifier;
use GatoExternalPrefixByGatoGraphQL\Doctrine\DBAL\Schema\Name\UnqualifiedName;
use GatoExternalPrefixByGatoGraphQL\Doctrine\DBAL\Schema\PrimaryKeyConstraint;
use GatoExternalPrefixByGatoGraphQL\Doctrine\DBAL\Schema\Schema;
use GatoExternalPrefixByGatoGraphQL\Doctrine\DBAL\Schema\Table;
...
class PdoSessionHandler extends AbstractSessionHandler
{
    ...
    private function buildSchemaTable() : Table
    {
        ...
        switch ($this->driver) {
            case 'mysql':
                $editor->addColumn(Column::editor()->setUnquotedName($this->idCol)->setTypeName(Types::BINARY)->setLength(128)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->dataCol)->setTypeName(Types::BLOB)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->lifetimeCol)->setTypeName(Types::INTEGER)->setUnsigned(\true)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->timeCol)->setTypeName(Types::INTEGER)->setUnsigned(\true)->setNotNull(\true)->create())->setOptions(['engine' => 'InnoDB']);
                break;
            case 'sqlite':
                $editor->addColumn(Column::editor()->setUnquotedName($this->idCol)->setTypeName(Types::TEXT)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->dataCol)->setTypeName(Types::BLOB)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->lifetimeCol)->setTypeName(Types::INTEGER)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->timeCol)->setTypeName(Types::INTEGER)->setNotNull(\true)->create());
                break;
            case 'pgsql':
                $editor->addColumn(Column::editor()->setUnquotedName($this->idCol)->setTypeName(Types::STRING)->setLength(128)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->dataCol)->setTypeName(Types::BINARY)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->lifetimeCol)->setTypeName(Types::INTEGER)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->timeCol)->setTypeName(Types::INTEGER)->setNotNull(\true)->create());
                break;
            case 'oci':
                $editor->addColumn(Column::editor()->setUnquotedName($this->idCol)->setTypeName(Types::STRING)->setLength(128)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->dataCol)->setTypeName(Types::BLOB)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->lifetimeCol)->setTypeName(Types::INTEGER)->setNotNull(\true)->create())->addColumn(Column::editor()->setUnquotedName($this->timeCol)->setTypeName(Types::INTEGER)->setNotNull(\true)->create());
                break;
        ...
        return $editor->addPrimaryKeyConstraint(new PrimaryKeyConstraint(null, [new UnqualifiedName(Identifier::unquoted($this->idCol))], \true))->addIndex(Index::editor()->setUnquotedName($this->lifetimeCol . '_idx')->setUnquotedColumnNames($this->lifetimeCol)->create())->create();
    }
    /**
    ...
    private function configureSchemaTable(Table $table) : void
    {
        switch ($this->driver) {
            case 'mysql':
                $table->addColumn($this->idCol, Types::BINARY, ['length' => 128, 'notnull' => \true]);
                $table->addColumn($this->dataCol, Types::BLOB, ['notnull' => \true]);
                $table->addColumn($this->lifetimeCol, Types::INTEGER, ['unsigned' => \true, 'notnull' => \true]);
                $table->addColumn($this->timeCol, Types::INTEGER, ['unsigned' => \true, 'notnull' => \true]);
                $table->addOption('engine', 'InnoDB');
                break;
            case 'sqlite':
                $table->addColumn($this->idCol, Types::TEXT, ['notnull' => \true]);
                $table->addColumn($this->dataCol, Types::BLOB, ['notnull' => \true]);
                $table->addColumn($this->lifetimeCol, Types::INTEGER, ['notnull' => \true]);
                $table->addColumn($this->timeCol, Types::INTEGER, ['notnull' => \true]);
                break;
            case 'pgsql':
                $table->addColumn($this->idCol, Types::STRING, ['length' => 128, 'notnull' => \true]);
                $table->addColumn($this->dataCol, Types::BINARY, ['notnull' => \true]);
                $table->addColumn($this->lifetimeCol, Types::INTEGER, ['notnull' => \true]);
                $table->addColumn($this->timeCol, Types::INTEGER, ['notnull' => \true]);
                break;
            case 'oci':
